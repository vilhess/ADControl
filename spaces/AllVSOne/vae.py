import sys
sys.path.append('/home/svilhes/Bureau/these/projections/AllVSOne')
...
for anormal in range(10):
    ...
    for digit, images in test_dict_dataset.items():
        ...
        if digit==ANORMAL:
            plt.scatter(z[:,0], z[:,1], c=labels[digit], label=f'{digit} -- ANORMAL')
        else:
            plt.scatter(z[:,0], z[:,1], c=labels[digit], label=digit)
    plt.legend()
    plt.title(f"2D latent space - anormal = {ANORMAL}.png")
    plt.savefig(f"figures/vae/latent_space_anom_{ANORMAL}.png")
    plt.close