...
for anormal in range(10):
    ...
    if 'fanogan' not in results.keys():
        results['fanogan']={}
    results["fanogan"][f"Anormal_{ANORMAL}"] = auc
    ...
    plt.savefig(f'results/figures/fanogan/mean_scores_{ANORMAL}.jpg', bbox_inches='tight', pad_inches=0)
    plt.close()
    ...
    with open(f"results/p_values/fanogan/pval_{ANORMAL}.json", "w") as file:
